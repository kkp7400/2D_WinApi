...
void Image::Render3(HDC hdc, int destX, int destY, bool isCenterRenderring, float maxPer, float nowPer)
{
    int x = destX;
    int y = destY;
    if (isCenterRenderring)
    ...
    if (isTransparent)
    {
        ...
        GdiTransparentBlt(
            hdc,
            x, y,
            (imageInfo->width / maxPer) * nowPer, imageInfo->height,
...
void Image::AlphaRender2(HDC hdc, int destX, int destY, bool isCenterRenderring, int currFrameX, float size)
{
    imageInfo->currFrameX = currFrameX;
    imageInfo->currFrameY = 0;
    int x = destX;
    ...
    AlphaBlend(
        ...
        0,0,
        imageInfo->frameWidth,
        imageInfo->frameHeight, blendFunc
    );  //BLENDFUNCTION()
