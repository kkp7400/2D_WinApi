...
inline bool RectToCircle(RECT rect, FPOINT p, int p2size)
{
	if (rect.top < p.y + p2size / 2 && rect.bottom > p.y - p2size / 2)
	{
		return true;
	}
	else
...
inline float Distance(FPOINT p1, FPOINT p2) {
	float distance;
...
inline bool CircleToCircle(FPOINT p1, FPOINT p2, int p1size, int p2size)
{
	if (Distance(p1, p2) < p1size / 2 + p2size / 2) {
		return true;
	}
	else
	{
